...
1. **`/grill`** sharpens the idea by interview. Start here whenever you are **working in a working directory**: it's stateful, retaining what it learns in `CONTEXT.md` and ADRs. (No working directory? Use `/grilling` instead, covered under Standalone. `grill` runs that same primitive and is the one that leaves a paper trail, which makes it the better of the two whenever a repo is there to leave it in.)
...
- **`/grilling`** is the interview primitive itself: rounds, the frontier, facts are the agent's job and decisions are yours. It's the same relentless interview as `/grill`, but **stateless**: it saves nothing locally and builds no `CONTEXT.md`. Reach for it when you are **not working in a working directory** (sharpening a plan, a design, a piece of writing, anything with no repo under it). If you are in a working directory, use `/grill` instead: it runs the same interview and leaves a paper trail, so it is strictly the better one. `/triage`, `/wayfinder` and `/deepen` all run it internally.
