...
These are canonical role names, the triage part of the repo's ticket lifecycle, which also defines `in-progress` and `done` for the skills that claim and close tickets. The actual label strings used in the issue tracker may differ. The lifecycle and its mapping should have been provided to you. If not, tell the user to run `/setup-repo`.
